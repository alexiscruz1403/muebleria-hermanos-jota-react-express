...
export const Login = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try{
            if(validateAll()){
                setLoading(true);
                const response = await login(formValues.email, formValues.contrasenia);
                onLoginSuccess(response.token, response.user);
                navigate("/");
            }
        }catch(error){
            console.error("Error al iniciar sesión:", error);
            setLoginError(error.response?.data?.message || "Error al iniciar sesión. Por favor, intentá nuevamente.");
        }finally{
            setLoading(false);
        }
    }
        ...
        <div className="p-4 bg-[#f4e4d0] flex items-center justify-center px-4">
            <div className="bg-[#f8f0e3] shadow-lg rounded-2xl p-8 w-full max-w-md border border-[#c9a27e]">
                ...
                <form className="space-y-5">
                    <div>
                        <label className="block text-[#7a4b2d] font-medium mb-1">
                            Correo electrónico
                        </label>
                        <input
                            type="email"
                ...
                <p className="text-center text-[#7a4b2d] mt-6">
                    ¿No tenés una cuenta?{" "}
                    <Link to="/register" className="font-semibold text-[#b4753d] hover:underline">
                        Registrate
                    </Link>
                </p>
            </div>
